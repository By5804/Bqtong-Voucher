...
export default function SoldVouchersPage() {
  ...
  const fetchVouchers = useCallback(async () => {
    ...
    let query = supabase
      .from('vouchers')
      .select('*', { count: 'exact' })
      .eq('status', 'sold') // Filter utama: hanya voucher yang terjual
  ...
  return (
    <div className="container mx-auto py-8">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-2xl font-bold">Data Voucher Terjual</h1>
      </div>
      
      <Card className="mb-8">
        <CardHeader><CardTitle>Filter Voucher Terjual</CardTitle></CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          <div className="flex justify-between items-center">
            <CardTitle>Daftar Voucher Terjual ({totalVouchers})</CardTitle>
          </div>
        </CardHeader>
        <CardContent>
          {loading ? (<p>Memuat data...</p>) : (
            <>
              ...
              {vouchers.length === 0 && !loading && (<div className="text-center py-10 text-gray-500"><p>Tidak ada voucher terjual yang cocok dengan filter Anda.</p></div>)}
